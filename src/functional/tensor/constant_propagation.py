...
class ConstantPropagation(eve.NodeTranslator):
    def visit_SymRef(self, node, *, constants):
        if (value := constants.get(node.id)) is not None:
            return _lit(value)
        return node
        ...
        if isinstance(node.fun, ir.Lambda):
            ...
            for param, arg in zip(node.fun.params, args):
                if isinstance(arg, ir.Literal):
                    inner_constants[param.id] = _unlit(arg)
                else:
            ...
            return ir.FunCall(
                fun=ir.Lambda(params=tuple(non_const_params), expr=expr), args=tuple(non_const_args)
            )

...
class PrettyPrinter(NodeTranslator):
    ...
    def visit_Slice(self, node: ir.Slice, *, prec: int) -> list[str]:
        slices = [
            [f"{dim}: {start}:{stop}"]
            for dim, start, stop in zip(node.dims, node.starts, node.stops)
        ]
        hslices = self._hmerge(["["], *self._hinterleave(slices, ", "), ["]"])
        vslices = self._vmerge(["["], *self._hinterleave(slices, ",", indent=True), ["]"])
        oslices = self._optimum(hslices, vslices)
        return self._hmerge(self.visit(node.expr, prec=PRECEDENCE["__slice__"]), oslices)

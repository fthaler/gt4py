"""A pretty printer for the tensor IR.
...
class PrettyPrinter(NodeTranslator):
    ...
    def visit_Sym(self, node: ir.Sym, *, prec: int) -> list[str]:
        return self._hmerge([node.id], [": "], self.visit(node.type, prec=0))
    ...
    def visit_OffsetType(self, node: ir.OffsetType, *, prec: int) -> list[str]:
        return ["offset"]

    def visit_Builtin(self, node: ir.Builtin, *, prec: int) -> list[str]:
        return ["__" + node.name]
    ...
    def visit_Dim(self, node: ir.Dim, *, prec: int) -> list[str]:
        return [f"{node.name}[{node.start}:{node.stop}]"]

    def visit_TensorType(self, node: ir.TensorType, *, prec: int) -> list[str]:
        return self._hmerge(
            ["tensor<"],
            *self._hinterleave(
                (self.visit(node.dtype, prec=0),) + self.visit(node.dims, prec=0), ", "
            ),
            [">"],
        )

    def visit_FunctionType(self, node: ir.FunctionType, *, prec: int, **kwargs) -> list[str]:
        args = self._hmerge(["("], *self._hinterleave(self.visit(node.args, prec=0), ", "), [")"])
        ret = self.visit(node.ret, prec=0)
        return self._hmerge(args, [" → "], ret)

    def visit_Lambda(self, node: ir.Lambda, *, prec: int) -> list[str]:
        params: Any = [
            self._hmerge([p.id], [": "], self.visit(p.type, prec=0)) for p in node.params
        ]
    ...
    def visit_FunCall(self, node: ir.FunCall, *, prec: int) -> list[str]:
        if isinstance(node.fun, ir.Builtin):
            fun_name = node.fun.name
        ...
        if isinstance(node.fun, ir.SymRef) and node.fun.id == "make_tuple":
    ...
    def visit_Fencil(self, node: ir.Fencil, *, prec: int) -> list[str]:
        assert prec == 0
        ...
        return self._vmerge(params, self._indent(closures), ["}"])
